module.exports = {
  meta: {
    ...
    docs: {
      ...
      description:
        "Enforce the use of logger.error() inside try and catch statements",
    },
    fixable: "code",
    schema: [],
  },
  create(context) {
    ...
    return {
      ...
      TryStatement(node) {
        ...
        const hasLoggerError = [firstStatement, lastStatement].every(
          (statement) => {
            return (
              statement.type === "ExpressionStatement" &&
              statement.expression.type === "CallExpression" &&
        ...
        if (!hasLoggerError) {
          context.report({
            node,
            message:
              "Use logger.info() at the beginning and end of try statements",
          });
        }
      },
    };
